...
alpha = float(sys.argv[3]); # specify alpha value in .sh file
N = int(alpha*d);
P = int(tvals[tauind]*(d**2));

trainobject = FiniteSampler(n_points = N+1, n_dims= d, eta_scale = sigma, w_scale = psi, diversity=2*d, batch_size = P, seed=None);
config = TransformerConfig(pos_emb=False, n_hidden=h, n_layers=lays)
